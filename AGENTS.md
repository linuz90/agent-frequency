...
- Leases expire automatically using the fixed `15m`, `30m`, `1h`, and `2h` buckets. A `state: "done"` announcement through the same tool releases the caller's lease and claims immediately; there is no separate completion tool. `state: "testing"` is the middle phase: the lease stays live, its scopes are recorded as `shared`, and they stop arbitrating so peers are unblocked while verification runs. Storage keeps the two v2 `agent_state` values and carries testing in an additive flag column, so live leases, the activity feed, and already-running processes survive the upgrade; because an older process renews a lease without knowing that column, a lease holding an exclusive claim is always read back as working.
